...
class SyncEntryModelsUseCase @Inject constructor(private val commandBus: CommandBus) {

    suspend operator fun invoke(userId: String): Answer<Unit, SyncEntriesReason> =
        SyncEntriesCommand(userId = userId).let { command -> commandBus.dispatch(command) }
